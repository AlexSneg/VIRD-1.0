...
namespace VncLib
{
    ...
    /// <summary>
        ...
        /// </summary>
    public class RemoteDesktop : UserControl // TODO: numerous places say that you should use Panel instead of ScrollableControl 
    {
        ...
        public RemoteDesktop() : base()
        {
            ...
            // Allow for partial viewing of the remote desktop
            //AutoScroll = true;
            //AutoScrollMinSize = new Size(608, 427); // just a default for Design graphic. Will get changed once connected.
			
            // Show a screenshot of a Windows desktop from the manifest and cache to be used when painting in design mode
            designModeDesktop = Image.FromStream(Assembly.GetAssembly(GetType()).GetManifestResourceStream("VncLib.Resources.screenshot.png"));
        ...
        protected double ScaleX
        {
            get
            {
                if (IsConnected && vnc != null)
                {
                    //return ClientSize.Width /(vnc.Framebuffer.Width * 1.0);
                    return ClientSize.Width / (ShownRectangle.Width * 1.0);
                }
                return 1;
            }
        }
                ...
                protected double ScaleY
        {
            get
            {
                if (IsConnected && vnc != null)
                {
                    //return ClientSize.Height / (vnc.Framebuffer.Height * 1.0);
                    return ClientSize.Height / (ShownRectangle.Height * 1.0);
                }
                return 1;
            }
        }
        ...
        protected Rectangle ShownRectangle
        {
            get
            {
                if (IsConnected && vnc != null)
                {
                    int width = vnc.Framebuffer.Width - _croppingLeft - _croppingRight;
                    int height = vnc.Framebuffer.Height - _croppingTop - _croppingBottom;
                    return new Rectangle(_croppingLeft, _croppingTop, width, height);
                }
        ...
        /// blocking.
        /// </remarks>
        /// <exception cref="System.InvalidOperationException">Thrown if the RemoteDesktop control is not in the Connected state.  See <see cref="VncLib.RemoteDesktop.IsConnected" />.</exception>
        ...
        protected void VncUpdate(object sender, VncEventArgs e)
        {
            ...
            // Before Invalidating the control, specify the rectangle that needs to be
            // re-painted.  This speeds OnPaint dramatically, since the rest of the 
            // screen doesn't need to change.  Also, adjust the invalidated rectangle
            // to accomodate scrolling or centering, since the desktop will either be 
            // clipped or centered on the screen.
            int x, y;
            //int desktopImageWidth = (int)(desktop.Size.Width * ScaleX);
            //int desktopImageHeight = (int)(desktop.Size.Height * ScaleY);
            int desktopImageWidth = ClientSize.Width;
            int desktopImageHeight = ClientSize.Height;

            // определим - то что нужно апдейтить вообще попадает в видимую часть
            Rectangle newUpdateRectangle = e.DesktopUpdater.UpdateRectangle;
            if (ShownRectangle.IntersectsWith(newUpdateRectangle))
            {
                newUpdateRectangle.Intersect(ShownRectangle);
                if (newUpdateRectangle.IsEmpty) return;
            }
            else
            {
                return;
            ...
            //if (ClientSize.Width > desktopImageWidth)
            //{
            //    int dx = (ClientRectangle.Width - desktopImageWidth) / 2;
            //    x = (int)(e.DesktopUpdater.UpdateRectangle.X * ScaleX) + dx;
            //}
            //else
            //{
            //    x = (int)(e.DesktopUpdater.UpdateRectangle.X * ScaleX) + AutoScrollPosition.X;
            //}

            //if (ClientSize.Height > desktopImageHeight)
            //{
            //    int dy = (ClientRectangle.Height - desktopImageHeight) / 2;
            //    y = (int)(e.DesktopUpdater.UpdateRectangle.Y * ScaleY) + dy;
            //}
            //else
            //{
            //    y = (int)(e.DesktopUpdater.UpdateRectangle.Y * ScaleY) + AutoScrollPosition.Y;
            //}

            x = (int)((newUpdateRectangle.Left - ShownRectangle.Left) * ScaleX);
            y = (int)((newUpdateRectangle.Top - ShownRectangle.Top) * ScaleY);

            int width = (int) (newUpdateRectangle.Width * ScaleX);
            int height = (int)(newUpdateRectangle.Height * ScaleY);

            Rectangle adjusted = new Rectangle(x, y, width, height);

            //Rectangle adjusted = new Rectangle(x,
            //                                   y,
            //                                   (int)(e.DesktopUpdater.UpdateRectangle.Width * ScaleX),
            //                                   (int)(e.DesktopUpdater.UpdateRectangle.Height * ScaleY));			

            //if (ClientSize.Width > desktop.Size.Width)
            //{
            //    int dx = (ClientRectangle.Width - desktop.Width) / 2;
            //    x = e.DesktopUpdater.UpdateRectangle.X + dx;
            //}
            //else
            //{
            //    x = e.DesktopUpdater.UpdateRectangle.X + AutoScrollPosition.X;
            //}

            //if (ClientSize.Height > desktop.Size.Height)
            //{
            //    int dy = (ClientRectangle.Height - desktop.Height) / 2;
            //    y = e.DesktopUpdater.UpdateRectangle.Y + dy;
            //}
            //else
            //{
            //    y = e.DesktopUpdater.UpdateRectangle.Y + AutoScrollPosition.Y;
            //}

            //Rectangle adjusted = new Rectangle(x,
            //                                   y,
            //                                   e.DesktopUpdater.UpdateRectangle.Width,
            //                                   e.DesktopUpdater.UpdateRectangle.Height);			
            Invalidate(adjusted, true);
            //Invalidate();
        ...
        {
            // Use Display 0 by default.
            Connect(host, 0, viewOnly,0,0,0,0);
        }
        ...
        {
            Connect(host, display, false,0,0,0,0);
        }
        ...
        /// <param name="display">The Display number (used on Unix hosts).</param>
        /// <param name="viewOnly">Determines whether mouse and keyboard events will be sent to the host.</param>
        /// <param name="croppingLeft"></param>
        /// <param name="croppingRight"></param>
        /// <param name="croppingTop"></param>
        /// <param name="croppingBottom"></param>
        /// <exception cref="System.ArgumentNullException">Thrown if host is null.</exception>
        /// <exception cref="System.ArgumentOutOfRangeException">Thrown if display is negative.</exception>
        /// <exception cref="System.InvalidOperationException">Thrown if the RemoteDesktop control is already Connected.  See <see cref="IsConnected" />.</exception>
        public void Connect(string host, int display, bool viewOnly, int croppingLeft, int croppingRight, int croppingTop, int croppingBottom)
        {
            _croppingBottom = croppingBottom;
            _croppingLeft = croppingLeft;
            _croppingRight = croppingRight;
            _croppingTop = croppingTop;
        ...
        /// </summary>
        /// <exception cref="System.InvalidOperationException">Thrown if the RemoteDesktop control is already Connected.  See <see cref="VncLib.RemoteDesktop.IsConnected" />.</exception>
        ...
        /// </summary>
        /// <exception cref="System.InvalidOperationException">Thrown if the RemoteDesktop control is already in the Connected state.  See <see cref="VncLib.RemoteDesktop.IsConnected" />.</exception>		
        protected void Initialize()
        {
            ...
            //AutoScrollMinSize = new Size((int)(vnc.Framebuffer.Width * ScaleX), (int)(vnc.Framebuffer.Height * ScaleY));
        ...
        /// </summary>
        /// <exception cref="System.InvalidOperationException">Thrown if the RemoteDesktop control is not already in the Connected state. See <see cref="VncLib.RemoteDesktop.IsConnected" />.</exception>
        protected void SetupDesktop()
        {
            ...
            int height = vnc.Framebuffer.Height - _croppingTop - _croppingBottom;
            if (width <= 0 || height <= 0)
                throw new ApplicationException("Параметры обрезки некорректны - окно схлопывается");
        ...
        public void DrawDesktopMessage(string message)
        {
        ...
        protected void DrawDesktopImage(Image desktopImage, Graphics g)
        {
            ...
            //int x, y;
            //int desktopImageWidth = (int)(desktopImage.Size.Width * ScaleX);
            //int desktopImageHeight = (int)(desktopImage.Size.Height * ScaleY);

            //if (ClientSize.Width > desktopImageWidth)
            //{
            //    x = (ClientRectangle.Width - desktopImageWidth) / 2;
            //}
            //else
            //{
            ...
            //if (ClientSize.Height > desktopImageHeight)
            //{
            //    y = (ClientRectangle.Height - desktopImageHeight) / 2;
            //}
            //else
            //{
            ...
            //g.DrawImage(desktopImage, x, y, desktopImageWidth, desktopImageHeight);

            //g.DrawImage(desktopImage, 0, 0, ClientSize.Width, ClientSize.Height);
            g.DrawImage(desktopImage, ClientRectangle, ShownRectangle, GraphicsUnit.Pixel);


            //if (ClientSize.Width > desktopImage.Size.Width)
            //{
            //    x = (ClientRectangle.Width - desktopImage.Width) / 2;
            //}
            //else
            //{
            //    x = DisplayRectangle.X;
            ...
            //if (ClientSize.Height > desktopImage.Size.Height)
            //{
            //    y = (ClientRectangle.Height - desktopImage.Height) / 2;
            //}
            //else
            //{
            //    y = DisplayRectangle.Y;
        ...
        protected override void OnMouseMove(MouseEventArgs mea)
        {
            ...
            if (IsConnected) {
                ...
                //    vnc.Framebuffer.Rectangle.Left,vnc.Framebuffer.Rectangle.Top,
                //    (int)(vnc.Framebuffer.Rectangle.Width * ScaleX),
                //    (int)(vnc.Framebuffer.Rectangle.Height * ScaleY));

                //if (ClientSize.Width > desktopImageWidth)
                //{
                //    desktopRect.X = (ClientRectangle.Width - desktopImageWidth) / 2;
                //}

                //if (ClientSize.Height > desktopImageHeight)
                //{
                //    desktopRect.Y = (ClientRectangle.Height - desktopImageHeight) / 2;
                //}

                Rectangle desktopRect = ClientRectangle;
                // See if the mouse pointer is inside the area occupied by the desktop on screen.
                if (desktopRect.Contains(PointToClient(MousePosition))) {
                    UpdateRemotePointer();


                    //Since there might be dead space surrounding the desktop, only bother sending
                    //the update if the mouse is really over the image
                    //Rectangle desktopRect = vnc.Framebuffer.Rectangle;

                    //if (ClientSize.Width > desktop.Size.Width) {
                    //    desktopRect.X = (ClientRectangle.Width - desktop.Width) / 2;
                    //}

                    //if (ClientSize.Height > desktop.Size.Height ) {
                    //    desktopRect.Y = (ClientRectangle.Height - desktop.Height) / 2;
                    //}
        ...
        protected override void OnMouseWheel(MouseEventArgs mea)
        {
            ...
            if (!DesignMode && IsConnected) {
                ...
                vnc.WritePointerEvent(mask, new Point(current.X, current.Y));
            }			
        ...
        private void UpdateRemotePointer()
        {
            ...
            if (!DesignMode && IsConnected) {
                ...
                // Adjust the mouse location relative to the position of the desktop,
                // which could be clipped and scrolled, or centered in a larger window.
                ...
                //{
                //    int dx = (ClientRectangle.Width - desktopImageWidth) / 2;
                //    adjusted.X = current.X - dx;
                //}
                //else
                //{
                //    adjusted.X = current.X - AutoScrollPosition.X;
                //}

                //if (ClientSize.Height > desktopImageHeight)
                //{
                //    int dy = (ClientRectangle.Height - desktopImageHeight) / 2;
                //    adjusted.Y = current.Y - dy;
                //}
                //else
                //{
                //    adjusted.Y = current.Y - AutoScrollPosition.Y;
                //}
                adjusted.X = current.X;
                adjusted.Y = current.Y;

                adjusted.X = (int)(adjusted.X/ScaleX) + ShownRectangle.Left;
                adjusted.Y = (int)(adjusted.Y/ScaleY) + ShownRectangle.Top;
                ...
                vnc.WritePointerEvent(mask, adjusted);
            }
        }
        ...
        public void SendSpecialKeys(SpecialKeys keys)
        {
            ...
            switch(keys) {
                case SpecialKeys.Ctrl:
                    PressKeys(new uint[] {0xffe3}, false);	// CTRL, but don't release
                    break;
                case SpecialKeys.Alt:
                    PressKeys(new uint[] {0xffe9}, false);	// ALT, but don't release
                    break;
                case SpecialKeys.CtrlAltDel:
                    PressKeys(new uint[] {0xffe3, 0xffe9, 0xffff}, true); // CTRL, ALT, DEL
                    break;
                case SpecialKeys.AltF4:
                    PressKeys(new uint[] {0xffe9, 0xffc1}, true); // ALT, F4
                    break;					
                case SpecialKeys.CtrlEsc:
                    PressKeys(new uint[] {0xffe3, 0xff1b}, true); // CTRL, ESC
                    break;
