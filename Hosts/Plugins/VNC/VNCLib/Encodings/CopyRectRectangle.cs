        ...
        public unsafe override void Draw(Bitmap desktop)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    //Fix
                    // если окно выходит за нижнюю границу клиентского окна то приложение падает
                    //int cutting = rectangle.Y + rectangle.Height - desktop.Height;
                    //if (cutting >= 0) cutting = 1;
                    //else cutting = 0;
                    //pSrc -= (cutting * desktop.Width);
                    //pDest -= (cutting * desktop.Width);

                    for (int y = 0; y < rectangle.Height/* - cutting*/; ++y)
                    {
                        for (int x = 0; x < rectangle.Width; ++x)
                        {
                            *(--pDest) = *(--pSrc);
