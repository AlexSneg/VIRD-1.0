    ...
    internal class DsoFramerHelper
    {
        ...
        private bool WindowEnumProc(IntPtr hwnd, int lParam)
        {
            ...
            if (text.Length > 0)
            {
                ...
                {
                    uint style = (uint)WordUtils.GetWindowLong(hwnd, WordUtils.GWL_STYLE);
                    if ((style & (uint)WordUtils.WindowStyles.WS_VISIBLE) != 0)
                    {
                        _webIsVisible = true;
        ...
        public void ResizeAxControl()
        {
            if (_webIsVisible)
            {
                ...
                WordUtils.MoveWindow(_axControl.Handle, x, y, _axControl.Width, _axControl.Height, true);
            }
        }
    }
}
